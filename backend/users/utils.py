...
async def get_all_users(current_user, db: Session = Depends(get_db)):
    ...
    dict_users = [{"id": user.id, "name": user.name, "roles": get_user_roles(user.id), "department_name": get_department_name_by_id(user.departament_id)} for user in all_users]
...
async def get_my_user(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    current_user.deparament_name = get_department_name_by_id(current_user.department_id)

    return ExtendedUserData(id=current_user.id, name=current_user.name, email=current_user.email, organization_id=current_user.organization_id, organization_name=current_user.organization_name, organization_address=current_user.organization_address, department_id=current_user.department_id, department_name=current_user.department_name, roles=get_user_roles(current_user.id))
